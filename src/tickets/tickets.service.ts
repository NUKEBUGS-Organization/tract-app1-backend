...
type AuthUser = { _id: { toString(): string }; role: Role };
...
export class TicketsService {
  ...
  async create(user: AuthUser, dto: CreateTicketDto) {
    ...
    const ticket = await this.ticketModel.create({
      ...
      userRole: user.role,
      sourceApp: TicketSourceApp.APP1,
    ...
    await this.notificationsService.dispatch({
      ...
      action_url: `/support/${ticket._id.toString()}`,
    });
  ...
  async listForUser(user: AuthUser, sourceApp?: TicketSourceApp) {
    ...
    if (user.role === Role.ADMIN && sourceApp) {
  ...
  async update(user: AuthUser, ticketId: string, dto: UpdateTicketDto) {
    ...
    if (dto.reply) {
      ...
      if (isAdmin) {
        await this.notificationsService.dispatch({
          recipient_id: ticket.userId.toString(),
  ...
  toPublic(ticket: SupportTicketDocument) {
    const doc = ticket.toObject() as SupportTicketDocument & {
      createdAt?: Date;
      updatedAt?: Date;
    };
    return {
      ...
      sourceApp: ticket.sourceApp ?? TicketSourceApp.APP1,
