...
export class UsageLimitService {
  constructor(
    ...
    private readonly counters: Model<UsageCounterDocument>,
    private readonly subscriptions: SubscriptionsService,
  ) {}

  private async used(userId: string, kind: UsageKind): Promise<number> {
    const row = await this.counters
      .findOne({ userId: new Types.ObjectId(userId), kind })
      .lean()
      .exec();
    return row?.used ?? 0;
  }
  ...
  async getAllowance(userId: string, kind: UsageKind): Promise<UsageAllowance> {
    ...
    return this.allowance(
      used,
      used >= FREE_LIMIT && !(await this.isActive(userId)),
    );
  }
  ...
  async consumeAttempt(
    ...
    if (!(await this.isActive(userId))) {
      throw new ForbiddenException({
        ...
        message:
          'Your 10 free lifetime attempts are used. Subscribe to continue.',
      });
    }
