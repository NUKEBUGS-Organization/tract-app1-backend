/** Same tier amounts as App2 — shared PayPal plans. */
...
export function paidThrough(
  ...
  if (!['ACTIVE', 'CANCELLED', 'EXPIRED'].includes(details.status ?? '')) {
    return null;
  }
  const payment = details.billing_info?.last_payment;
  if (
    payment?.amount?.currency_code !== 'USD' ||
    Number(payment.amount.value) !== amount
  ) {
    return null;
  }
  const paidAt = new Date(payment.time ?? '');
  if (!Number.isFinite(paidAt.getTime()) || paidAt.getTime() > Date.now() + 60_000) {
    return null;
  }
  ...
  return Number.isFinite(next.getTime()) && next > paidAt && next < end
    ? next
    : end;
}
