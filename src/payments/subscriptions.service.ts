...
export class SubscriptionsService {
    ...
    return {
      ...
      canCancel: Boolean(
        row?.paypalSubscriptionId &&
          ['ACTIVE', 'SUSPENDED', 'APPROVAL_PENDING', 'APPROVED'].includes(
            row.status,
          ),
      ),
  ...
  async getStatus(userId: string, refresh = false) {
    ...
    if (
      ...
      (refresh ||
        !row.syncedAt ||
        Date.now() - row.syncedAt.getTime() > 60_000)
    ) {
  ...
  async mockCheckout(userId: string) {
    if (this.subscriptionMode() !== 'mock') {
      throw new ForbiddenException(
        'Mock checkout is only available in mock mode.',
      );
    }
    const amount = await this.tier(userId);
    if (amount === null) {
  ...
  async create(userId: string, termsVersion: string) {
    if (termsVersion !== BETA_TERMS_VERSION) {
      throw new BadRequestException(
        'Accept the current subscription terms to continue.',
      );
    }
    if (this.subscriptionMode() === 'mock') {
      return this.mockCheckout(userId);
    }
    const amount = await this.tier(userId);
    if (amount === null) {
      throw new BadRequestException('Your role does not require a subscription.');
    ...
    if (!planId) {
      throw new ServiceUnavailableException(
        'Subscriptions are not configured yet. Please contact support.',
      );
    }
    let row: SubscriptionDocument | null = await this.subscriptions
      .findOne({ userId })
    ...
    if (row?.paypalSubscriptionId) {
      row = await this.sync(row);
      if (this.isPaid(row, row.amount)) {
        return { ...this.result(row, amount), approvalUrl: null };
      }
      if (
        ...
        if (row.amount !== amount) {
          throw new BadRequestException(
            'Cancel your previous subscription before changing tiers.',
          );
        }
    ...
    const details = await this.paypal.subscriptionRequest<PaypalSubscriptionDetails>(
      ...
      '/v1/billing/subscriptions',
      {
        ...
        application_context: {
          ...
          return_url: `${origin}/settings/subscription?paypal=return`,
          cancel_url: `${origin}/settings/subscription?paypal=cancel`,
        },
      },
      row.requestId,
    );
    if (!details.id) {
      throw new ServiceUnavailableException(
        'PayPal did not return a subscription. Please retry.',
      );
    }
    const approvalUrl =
      details.links?.find((link) => link.rel === 'approve')?.href ?? null;
    await this.subscriptions
      .updateOne(
        { _id: row._id, requestId: row.requestId },
        {
          $set: {
  ...
  private async sync(row: SubscriptionDocument): Promise<SubscriptionDocument> {
    const details =
      await this.paypal.subscriptionRequest<PaypalSubscriptionDetails>(
        'GET',
        `/v1/billing/subscriptions/${encodeURIComponent(row.paypalSubscriptionId!)}`,
      );
    if (
      details.id !== row.paypalSubscriptionId ||
      details.custom_id !== row.userId.toString() ||
      details.plan_id !== row.planId
    ) {
      throw new ForbiddenException(
        'PayPal subscription ownership or plan does not match.',
      );
    }
    const lastPaymentAt = new Date(
      details.billing_info?.last_payment?.time ?? '',
    );
    const revoked =
      row.revokedPaymentAt &&
      (!Number.isFinite(lastPaymentAt.getTime()) ||
        lastPaymentAt <= row.revokedPaymentAt);
    const updated = await this.subscriptions
      .findOneAndUpdate(
        {
          _id: row._id,
          paypalSubscriptionId: row.paypalSubscriptionId,
          revokedPaymentAt: row.revokedPaymentAt ?? null,
        },
        {
          $set: {
            status: details.status ?? 'UNKNOWN',
            paidUntil: revoked ? null : paidThrough(details, row.amount),
            lastPaymentAt: Number.isFinite(lastPaymentAt.getTime())
              ? lastPaymentAt
              : null,
            approvalUrl:
  ...
  async cancel(userId: string) {
    ...
    if (!row?.paypalSubscriptionId && row?.status !== 'PAID_TEST') {
    ...
    if (!['CANCELLED', 'EXPIRED'].includes(row.status)) {
      await this.paypal.subscriptionRequest(
        'POST',
        `/v1/billing/subscriptions/${encodeURIComponent(row.paypalSubscriptionId)}/cancel`,
        { reason: 'Cancelled by the subscriber in TRACT Seller.' },
      );
      await this.subscriptions
        .updateOne(
          {
